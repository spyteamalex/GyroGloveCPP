...
void SelectorRing::drawSector(int i, QPainter &painter, bool isSelected = false) {
    ...
    painter.drawPath(drawRing(width() / 2, height() / 2, innerRadius, outerRadius, startAngle + (ang * i + sectorBuf),
                              ang - 2 * sectorBuf));
...
SelectorRing::SelectorRing(double innerRadius, double outerRadius, double startAngle, double finishAngle,
                           QSize iconSize) : QWidget(nullptr),
...
#ifdef SECTOR_DISTANCE_DEFAULT
    sectorBuf = SECTOR_DISTANCE_DEFAULT;
#endif
#ifdef SECTOR_COLOR_DEFAULT
    sectorColor = SECTOR_COLOR_DEFAULT;
#endif
#ifdef SECTOR_BORDER_DEFAULT
    sectorBorder = SECTOR_BORDER_DEFAULT;
#endif
#ifdef SELECTED_SECTOR_BORDER_DEFAULT
    selectedSectorBorder = SELECTED_SECTOR_BORDER_DEFAULT;
#endif
#ifdef SELECTED_SECTOR_COLOR_DEFAULT
    selectedSectorColor = SELECTED_SECTOR_COLOR_DEFAULT;
#endif
...
void SelectorRing::setData(const QVector<SelectorRingItem*> &v) {
    SelectorRing::data = QVector<SelectorRingItem*>(v);
    update();
}
...
SelectorRingItem::SelectorRingItem(QObject *parent, QIcon icon, std::function<void(void)> action) : icon(std::move(icon)),
...
SelectorRingItem::SelectorRingItem(const SelectorRingItem &ringItem) : icon(ringItem.icon), action(ringItem.action){}
